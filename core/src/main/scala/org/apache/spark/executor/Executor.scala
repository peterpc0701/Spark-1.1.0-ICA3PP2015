  ...
  extends Logging
{
  ...
  def launchTask(
      context: ExecutorBackend, taskId: Long, taskName: String, serializedTask: ByteBuffer, cores: Int) {
    val tr = new TaskRunner(context, taskId, taskName, serializedTask, cores)
  ...
  class TaskRunner(
      execBackend: ExecutorBackend, val taskId: Long, taskName: String, serializedTask: ByteBuffer, cores: Int )
    extends Runnable {
    ...
    override def run() {
      ...
      try {
        ...
        val usableMemory = maxMemory - totalMemory + freeMemory
        ...
        /** initial value */
        var actualCoreNum = cores   
        if(spilledMemoryBytes > 0 && cores > 0){
          var maxComputingMemory = (maxMemory * memoryFraction * safetyFraction)
          if(totalMemory < maxMemory)
             maxComputingMemory = (totalMemory * memoryFraction * safetyFraction)
          actualCoreNum = (maxComputingMemory / (maxComputingMemory / cores + spilledMemoryBytes)).toInt
          if(actualCoreNum < 1)
            actualCoreNum = 1
        } 
       
        execBackend.statusUpdate(taskId, TaskState.FINISHED, serializedResult, actualCoreNum)
