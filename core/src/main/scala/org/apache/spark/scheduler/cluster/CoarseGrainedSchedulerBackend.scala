  ...
  extends SchedulerBackend with Logging
{
  ...
  class DriverActor(sparkProperties: Seq[(String, String)]) extends Actor with ActorLogReceive {
    ...
    private val actualNeededCores = new HashMap[String, Int]
    private val sampleCoresCounter = new HashMap[String, Int]
    private val sumOfSampleCores= new HashMap[String, Int]
    private val sampleNum= new HashMap[String, Int]
    ...
    def receiveWithLogging = {
      case RegisterExecutor(executorId, hostPort, cores) =>
        ...
        if (executorActor.contains(executorId)) {
          ...
          actualNeededCores(executorId) = cores
           sampleCoresCounter(executorId) = 0
          sumOfSampleCores(executorId) = 0
          sampleNum(executorId) = cores
      ...
      case StatusUpdate(executorId, taskId, state, data, actualCores) =>
        scheduler.statusUpdate(taskId, state, data.value)
        if (TaskState.isFinished(state)) {
          if (executorActor.contains(executorId)) {
            freeCores(executorId) += scheduler.CPUS_PER_TASK
           
             sampleCoresCounter(executorId) += 1
              sumOfSampleCores(executorId) += actualCores
              if(sampleCoresCounter(executorId) == sampleNum(executorId)){
                actualNeededCores(executorId) = sumOfSampleCores(executorId) /sampleNum(executorId)
                sampleNum(executorId) = actualNeededCores(executorId)
                sumOfSampleCores(executorId) = 0
                sampleCoresCounter(executorId) = 0
              }
              else{
                if(actualNeededCores(executorId) > actualCores)
                  actualNeededCores(executorId) = actualCores
              }
            if(totalCores(executorId) - freeCores(executorId) < actualNeededCores(executorId)) {
               makeOffers(executorId)
            }
            else{
              logInfo("( "+executorId+", "+totalCores(executorId)+", "+freeCores(executorId)+", "+actualCores+ ")")
            }    
        } else {
    ...
    def makeOffers() {
      launchTasks(scheduler.resourceOffers(
        executorHost.toArray.map {case (id, host) => new WorkerOffer(id, host, actualNeededCores(id) + freeCores(id) - totalCores(id))}))
    }

    // Make fake resource offers on just one executor
    def makeOffers(executorId: String) {
      launchTasks(scheduler.resourceOffers(
        Seq(new WorkerOffer(executorId, executorHost(executorId), actualNeededCores(executorId) + freeCores(executorId) - totalCores(executorId)))))
    }
    ...
    def removeExecutor(executorId: String, reason: String) {
      if (executorActor.contains(executorId)) {
        ...
        actualNeededCores -= executorId
         sampleCoresCounter -= executorId
        sumOfSampleCores -= executorId
        sampleNum -= executorId
